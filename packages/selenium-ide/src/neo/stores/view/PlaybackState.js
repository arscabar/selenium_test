...
class PlaybackState {
  ...
  @action.bound
  setCommandState(commandId, state, message, returnObj) {
    if(state == PlaybackStates.Failed || state == PlaybackStates.Fatal || state == PlaybackStates.Passed){
      browser.runtime.sendMessage({type : commandId , actionResult : state , returnObj : returnObj})
    }
    if (
