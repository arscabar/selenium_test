...
function logPromiseResult(functionName, promise) {
  return promise.then(result => {
    return result
  }).catch(error => {
    console.error(`Function ${functionName} threw an error:`, error)
    throw error
  })
}
    ...
    if (result.result !== 'success') {
      ...
      if (isElementNotFound(result.result)) {
        ...
        if (PlaybackState.playbackOptions.assertionsDisabled) {
          PlaybackState.setCommandState(id, PlaybackStates.Passed, result.returnObj)
        } else {
...
function getImplicitTimeout() {
  return 5000//isProduction ? 30000 : 5000
}
