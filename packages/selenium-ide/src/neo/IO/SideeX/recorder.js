...
export default class BackgroundRecorder {
  ...
  addCommandMessageHandler(message, sender, sendResponse) {
    try{
      ...
      {
        commandVal = {value : message.value , playState: message.playState}
      }
      else
      {
        commandVal = message.value
      }
      if (
        !message.command ||
      ...
      if (
        ...
        browser.windows
          ...
          .then(() => {
            ...
            setTimeout(() => {
              commandVal = prompt('Enter the name of the variable')
              if (message.insertBeforeLastCommand) {
                record(message.command, message.target, commandVal, true)
              } else {
                ...
                record(message.command, message.target, commandVal)
              }
            }, 100)
          })
        return
  ...
  async attach(startUrl) {
    ...
    try {
      ...
      await this.attachToExistingRecording("https://www.google.com")
  ...
  async attachToExistingRecording(url) {
    let testCaseId = getSelectedCase().id
    let windowExits = await this.isWindowExists(this.windowSession.currentUsedWindowId[testCaseId])
    try {
      if (windowExits && this.windowSession.currentUsedWindowId[testCaseId]) {
      ...
      } else if (windowExits && this.windowSession.generalUseLastPlayedTestCaseId === testCaseId) {
        ...
        const targetTab = allTabs.filter(tab => tab.id !== currentTab.id)[0]
        console.log(allTabs)
        console.log(targetTab)
        if (targetTab) {
          await this.createNewRecordingWindow(targetTab, testCaseId, url)
        }else{
          await this.createNewRecordingWindow(null, testCaseId, url)
        }
      }
    } catch (e) {
      console.log(e)
  ...
  async createNewRecordingWindow(tab, testCaseId, url) {
    try{
      if (tab == undefined || tab == null)
      {
        let windowInfo = await browser.windows.create({ url });
        if (windowInfo && windowInfo.tabs && windowInfo.tabs.length > 0) 
        {
