...
import playCommand from '../../../api/index'
...
export async function removeLastCommand() {
  const test = UiState.displayedTest
  while (test.commands.length > 0) 
  {
...
export default function record(
  ...
  try
  {
    ...
    if (command !== 'open') {
      ...
      newCommand = recordCommand(command, targets[0][0], value, index)
      if (Commands.list.has(command)) {
        ...
        if (type && type.name === ArgTypes.locator.name) {
          ...
          browser.runtime.sendMessage({recordData : newCommand})
        }
      }
    }
    if (PlayState != null && PlayState != undefined && PlayState == true)
    {
      playCommand({type: "PlayAction" , length : test.commands.length, command : newCommand } )
    }
  }
  catch(exception)
  {
