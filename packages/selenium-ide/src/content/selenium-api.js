...
Selenium.prototype.ScrapTable = function(tableCellAddress) {
  /**
...
Selenium.prototype.doScrapTable = function(tableName) {
  return new Promise((resolve, reject) => {
    let table = this.browserbot.findElement(tableName);

    if (!table || table.nodeName.toLowerCase() !== 'table') {
      table = findClosestTable(table)
    }

    if (!table) {
      reject(new SeleniumError('No table found for: ' + tableName))
      return;
    }
...
//   if (!table) {
//     // browser.runtime.sendMessage({type : "doScrapTable",actionResult : "fail", returnObj: null})
//     throw new SeleniumError('No table found for: ' + tableName)
//   }
//   let rows = table.querySelectorAll("tr");
//   let dataTable = {
//     columns: [],
...
//     }
//     if (i === 0) {
//       dataTable.columns = rowData;  // 첫 번째 행을 columns로 설정
//     } else {
//       dataTable.rows.push(rowData);
//     }
//   }
  
//   // browser.runtime.sendMessage({type : "doScrapTable", actionResult : "success", returnObj: dataTable})
//   return dataTable;
// }


Selenium.prototype.doGetImage = async function(imageLocator) {
  try
  {
    let imageElement = this.browserbot.findElement(imageLocator) // 이부분이 오래걸리나?
  
    if (!imageElement || imageElement.nodeName.toLowerCase() !== 'img') {
      throw new SeleniumError('No image found for: ' + imageLocator)
    }
    browser.runtime.sendMessage({ action: "copyImageToClipboard", imageUrl: imageElement.src }, function(response) {
      if (response && response.status === "success") 
      {
        copyImageToClipboard(response.image).then(() => 
        {
          console.log("Image copied to clipboard from content script.")
        }).catch(err => 
          {
          console.error("Failed to copy image in content script:", err)
        })
      } 
      else 
      {
        console.error("Error Occured in copy image:", response.message)
      }
    })
  }
  catch(exception)
