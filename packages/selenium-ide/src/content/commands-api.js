            ...
            returnValue
              .then(function(resolvedata) {
                ...
                if (!resolvedata) {
                  ...
                  sendResponse({ result: 'success', returnObj: resolvedata })
              }
              })
...
if (!window._listener) {
  ...
  browser.runtime.onMessage.addListener(async (message) => {
      try{
        if (message.addCommand)
        {
          await browser.runtime.sendMessage({type: "FROM_CONTENT", payload: "RecordingStart" , command : message.addCommand})
          console.log(`previous record ${message.addCommand.command}`)
          await record(message.addCommand.command, message.addCommand.target, {value: message.addCommand.value , playState: true})
          console.log(`after record ${message.addCommand.command}`)
          return true
        }
      }
      catch(exception){
